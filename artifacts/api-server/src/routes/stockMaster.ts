...
function generateItemCode(category: string, size: string, length: string): string {
  const cat = category.replace(/[^A-Z0-9]/gi, "").toUpperCase().slice(0, 6);
  const sz = size.replace(/[^A-Z0-9X]/gi, "").toUpperCase().slice(0, 6);
  const ln = length.replace(/[^A-Z0-9]/gi, "").toUpperCase().slice(0, 4);
  return `${cat}-${sz}-${ln}`;
}
...
router.get("/stock-master", authenticate, async (req, res): Promise<void> => {
  ...
  if (status) conditions.push(eq(stockMasterTable.status, status as "active" | "inactive"));
  if (search) {
    conditions.push(
      or(
  ...
  if (conditions.length > 0) {
    query = query.where(conditions.length === 1 ? conditions[0] : sql`${conditions[0]}${conditions.slice(1).map(c => sql` AND ${c}`).reduce((a, b) => sql`${a}${b}`)}`);
  }

  const items = await query.orderBy(stockMasterTable.category, stockMasterTable.size);
  res.json(items);
});

router.post("/stock-master", authenticate, requireRole("admin"), async (req, res): Promise<void> => {
  const parsed = CreateStockItemBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const { category, size, length, unit, status } = parsed.data;
  ...
  if (existing) {
    res.status(400).json({ error: "A stock item with this Category + Size + Length already exists" });
    return;
  }
  ...
  const [item] = await db
    ...
    .values({ category, size, length, unit: unit ?? "NOS", status: status ?? "active", itemCode })
    .returning();
...
router.get("/stock-master/categories", authenticate, async (_req, res): Promise<void> => {
  const rows = await db
...
router.get("/stock-master/:id", authenticate, async (req, res): Promise<void> => {
  const params = GetStockItemParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  }
  const [item] = await db.select().from(stockMasterTable).where(eq(stockMasterTable.id, params.data.id));
  if (!item) {
...
router.patch("/stock-master/:id", authenticate, requireRole("admin"), async (req, res): Promise<void> => {
  ...
  const [old] = await db.select().from(stockMasterTable).where(eq(stockMasterTable.id, params.data.id));
  if (!old) {
    res.status(404).json({ error: "Stock item not found" });
    return;
  }
...
router.delete("/stock-master/:id", authenticate, requireRole("admin"), async (req, res): Promise<void> => {
